...
public class GoodDaoImpl implements GoodDao {
    ...
    public Good getGood(String name) throws SQLException {
        Good good = null;
        try (PreparedStatement st = connection.prepareStatement(SELECT_GOOD_SQL_STATEMENT)) {
            st.setString(1, name);
    ...
    public Good getGood(Long goodId) throws SQLException {
        Good good = null;
        try (PreparedStatement st = connection.prepareStatement(SELECT_GOOD_BY_ID_SQL_STATEMENT)) {
            st.setLong(1, goodId);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
                good = new Good(rs.getLong("id"), rs.getString("name"), rs.getDouble("price"));////
    ...
    public List<Good> getAllGoods() throws SQLException {
        ...
        try (PreparedStatement st = connection.prepareStatement(SELECT_ALL_GOODS_SQL_STATEMENT);
             ResultSet rs = st.executeQuery()) {
